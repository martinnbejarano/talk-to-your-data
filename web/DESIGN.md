...
### Series chart
- **When it exists:** only when the answer is a series (D-16) — intakes by month, amount by currency, alerts by status. A single-number answer never carries one and never offers one, so the chart is not a mode the screen can be put into; it is what a series-shaped answer looks like.
- **Placement:** above, beside the answer sentence — never behind "Mostrar más" — with its own table expanded underneath. The drawing is for reading the shape; the figures are for defending the number to an auditor, and that is not done through a tooltip. The expanded table is also what makes the chart readable to a screen reader, so it is not an optional courtesy.
- **Marks:** bars for a categorical x, a line for a temporal one, and the choice is derived from the column type rather than chosen. No pie: a pie asserts that the parts are the whole, and here they almost never are — there are the soft-deleted, the approved with no intake date, the pending and the reversed.
...
- **Currency:** one panel per currency, each with its own scale and label. Never two currencies on one axis (D-08) — it is the same rule the ledger already obeys when it refuses to let a scale cross a unit change.
...
- **Library:** visx (`scale`, `axis`, `shape`, `group`) — SVG in React, so the existing CSS styles it and the marks stay DOM nodes, and its weight scales with what is imported (27 KB gzip against Recharts' 114 KB for the same two chart types). Not a canvas library.
...
- **Do** start every chart's value axis at zero, and give each currency its own panel with its own scale.
...
- **Don't** give a chart an "other" bucket, and don't draw one over `filas.muestra` — the table may be a sample, the chart never is.
