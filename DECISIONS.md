...
## D-16 · El gráfico es la forma de una respuesta que ya era una serie

**Qué decidimos.** El campo `grafico` del contrato, reservado en H3 y en `null` desde
entonces, se llena **sólo cuando la respuesta es una serie** —altas por mes, monto por
moneda, alertas por estado—. No es un adorno que se le agrega a una respuesta escalar: es
la forma que toma una respuesta que ya venía siendo una serie, y por eso va **arriba, con
la oración**, con su tabla desplegada debajo. Donde la respuesta es un número, no hay
gráfico ni ofrecido.
...
| Barras o línea | El sistema, por el tipo de la columna `x`: temporal → línea, categórica → barras |
| Si el mapeo es dibujable | El loop, determinístico: columna que no existe, o eje `y` no numérico → `grafico` vuelve a `null` |
...
consulta ya devolvió, y los valores se leen de `filas`. Inventar un punto no queda
prohibido por una instrucción del prompt ni atrapado por un validador: queda
**estructuralmente imposible**, que es la misma estrategia con la que se defiende D-04.
`cifras_sin_respaldo` no necesita aprender a mirar el gráfico.
...
([`NOTES/05-graficos.md`](NOTES/05-graficos.md) §1.1). Como el gráfico no entra al eval,
...
- **Sólo hay gráfico si las filas vinieron enteras.** `filas.muestra` son cinco sobre
  ...
  significar doce meses y no "los doce que entraron". Es un problema propio —nadie
  grafica sobre una muestra— y la única fuente que al menos declara el truncado en su
  contrato es Genie, con `query_result_metadata.is_truncated`.
- **Barras y línea. Sin torta.** Una torta afirma que las partes son el todo, y acá casi
  ...
  lo revertido.
- **Un panel por moneda, jamás dos en un eje.** Es D-08 dibujado. Compartir el eje de
  valores es sumar visualmente lo que los números no suman: un pico en pesos aplasta al
  dólar contra el piso y el ojo lee "no hubo dólares".
...
- **Tope de treinta puntos, y si se pasa se dice.** Doscientas barras no son un gráfico,
  son una textura. Y "otros" queda prohibido: esa suma la haría el front y no una
  consulta, que es D-04 dibujado.
- **El gráfico cede ante la derivación.** Una respuesta con serie necesita dos consultas
  —la cascada que `_lo_que_falta` exige con todos los escalones en una fila, y la
  agrupada—, contra un tope de doce pasos y un gate que rechaza justo la forma agrupada.
  Si no entra, se contesta sin gráfico: **el gráfico nunca puede costar la respuesta.**

**Qué cuesta.**

- `Ok` hoy descarta los tipos de columna —`columnas = [c.name for c in cursor.description]`,
  en [`core/db/ejecucion.py`](core/db/ejecucion.py)—. Deducir barras o línea exige que los
  lleve, y eso toca el seam y sus tests.
- Agregar `grafico` al esquema estricto lo cambia para las 33 preguntas y no sólo para las
  que grafican: la línea de base de D-14 y los diez reportes de `evals/reports/` quedan
  medidos contra un prompt anterior. Revalidar son US$ 10,70 y unos doce minutos.
- Una librería, en un front que hoy no tiene ninguna dependencia visual: **visx, 27 KB
  gzip**, la única donde el peso escala con lo que se usa (`@visx/scale` 12,3 +
  `@visx/axis` 17,2 + `@visx/shape` 3,6). Recharts eran 114 KB por los mismos dos tipos de
  gráfico, y su v3 no bajó respecto de la v2 (`NOTES/05-graficos.md` §5).

**Qué queda sin medir, dicho para que no aparezca después.** El gráfico **no entra al set
de evaluación**. Con la validación del loop, uno imposible no llega a la pantalla; uno
innecesario sí, y va arriba de todo. En producción no lo evalúa nadie —LangSmith, Ragas,
DeepEval, promptfoo, Genie y Cortex miden SQL o texto—, pero en investigación sí, y
nvBench 2.0 abandonó *accuracy* por P/R/F1@K porque más del 60 % de los casos son
ambiguos: el mismo problema que ya ordena este repo.
...
**Cuándo.** Después de las siete contestables que fallan las tres veces (D-14). Y en este
orden: los documentos, después el contrato con su baranda y sus tests **sin dibujar
nada**, y recién entonces la pantalla.
